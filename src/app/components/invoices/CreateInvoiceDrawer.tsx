...
export default function CreateInvoiceDrawer({
  ...
  // Document preview and real file upload modal state
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-xs">
            {invoiceDocEntries.length === 0 ? (
              <div className="p-12 text-center space-y-2">
                ...
                <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                  Use "+ Upload" above to attach payment receipts, proof files, or documents for this invoice.
                </p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse text-xs">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">
                    Attach files, receipts, or insurance proofs for {selectedClient?.name || "this invoice"}
                  </p>
                </div>
              </div>
              <button
                ...
                onClick={() => {
                  ...
                  addActivityEntry({
                    ...
                    details: {
                      ...
                      secondary: `File: ${fileName}${uploadFile ? ` (${(uploadFile.size / 1024).toFixed(1)} KB)` : ""} · Uploaded on ${new Date().toLocaleDateString()} · ${selectedClient?.name || "Client"}`,
                    },
                  });

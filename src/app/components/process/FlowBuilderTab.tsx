    ...
    if (draggingId) {
      ...
      if (draggedNode && draggedNode.config?.autoGenerated && onWorkflowStepsChange) {
        const getLaneKeyFromX = (x: number): "stage" | "incall" | "inchat" | "postcall" => {
          ...
          const distInChat = Math.abs(x - (BASE_X + 2 * LANE_X_SPACING));
          const distPostCall = Math.abs(x - (BASE_X + 3 * LANE_X_SPACING));
          const minDist = Math.min(distStage, distInCall, distInChat, distPostCall);
          if (minDist === distStage) return "stage";
          if (minDist === distInCall) return "incall";
          if (minDist === distInChat) return "inchat";
          return "postcall";
        };
        ...
        nodesWithLanes.forEach(n => {
          if (n.lane in lanes) lanes[n.lane as keyof typeof lanes].push(n);
        });
        ...
        (["stage", "incall", "inchat", "postcall"] as const).forEach(laneKey => {
          ...
          sorted.forEach((n, idx) => {
            ...
            if (originalStep) {
              ...
              if (laneKey === "incall" || laneKey === "inchat") {
                executionType = "wait";
              } else if (idx === 0) {
                executionType = "wait";
              } else {

...
export default function FlowBuilderTab({
  ...
  useEffect(() => {
        ...
        stepsInLane.forEach((step) => {
          ...
          if (group) {
            ...
            group.steps.forEach((member, memberIdx) => {
              ...
              // Wait node (delay > 0, non-parallel)
              ...
              if (member.params?.conditionsEnabled) {
                ...
                const condY = mY + (memberHasDelay ? 90 : 0);
                ...
                generatedNodes.push({
                  ...
                  stepKey: member.stepKey,
                  x: existingMember?.x ?? mX,
                  y: existingMember?.y ?? (mY + (memberHasDelay ? 90 : 0)),
                  config: {
                    ...(member.params ?? {}),
                    ...(existingMember?.config ?? {}),
                    autoGenerated: true,
            ...
            yCursor += anyMemberHasConditions ? 180 : 100;
          } else {
    ...
    setConnections(prevConnections => {
      const manualConnections = prevConnections.filter(
        c => !c.id.startsWith("auto-") && !c.id.startsWith("cond-") && !c.id.startsWith("parallel-") && !c.id.startsWith("wait-")
      );
      ...
      (["stage", "incall", "postcall"] as const).forEach(laneKey => {
        ...
        const processedStepIds = new Set<string>();
        let currentPredecessorId = "start";
        let currentPredecessorPort = "default";
        ...
        stepsInLane.forEach((step) => {
          ...
          if (group) {
            ...
            autoConnections.push({
              id: `parallel-${parallelNodeId}`,
              fromId: currentPredecessorId,
              fromPort: currentPredecessorPort,
              toId: parallelNodeId,
            });
            ...
            group.steps.forEach((member, memberIdx) => {
              ...
              // branch → wait? → cond? → step
              let memberFromId = parallelNodeId;
              let memberFromPort = branchPort;

              if (memberHasDelay) {
                autoConnections.push({ id: `wait-${memberWaitId}`, fromId: memberFromId, fromPort: memberFromPort, toId: memberWaitId });
                memberFromId = memberWaitId;
                memberFromPort = "default";
              }

              if (member.params?.conditionsEnabled) {
                autoConnections.push({ id: `cond-${memberCondId}`, fromId: memberFromId, fromPort: memberFromPort, toId: memberCondId });
                autoConnections.push({ id: `auto-${laneKey}-${member.id}`, fromId: memberCondId, fromPort: "default", toId: member.id });
              } else {
                autoConnections.push({ id: `auto-${laneKey}-${member.id}`, fromId: memberFromId, fromPort: memberFromPort, toId: member.id });
              }
            });

            currentPredecessorId = group.steps[0].id;
            currentPredecessorPort = "default";
          } else {
            processedStepIds.add(step.id);
            ...
            let seqFromId = currentPredecessorId;
            let seqFromPort = currentPredecessorPort;

            if (stepHasDelay) {
              autoConnections.push({ id: `wait-${stepWaitId}`, fromId: seqFromId, fromPort: seqFromPort, toId: stepWaitId });
              seqFromId = stepWaitId;
              seqFromPort = "default";
            }

            if (step.params?.conditionsEnabled) {
              autoConnections.push({ id: `cond-${stepCondId}`, fromId: seqFromId, fromPort: seqFromPort, toId: stepCondId });
              autoConnections.push({ id: `auto-${laneKey}-${step.id}`, fromId: stepCondId, fromPort: "default", toId: step.id });
            } else {
              autoConnections.push({ id: `auto-${laneKey}-${step.id}`, fromId: seqFromId, fromPort: seqFromPort, toId: step.id });
            }

            currentPredecessorId = step.id;
            currentPredecessorPort = "default";
          }
        });
      });
  ...
  const renderConfigFields = () => {
    ...
    // For auto-generated step nodes, render trigger/execution/delay controls + StepParametersFields
    if (configNode.config?.autoGenerated && configNode.stepKey) {
      const { autoGenerated: _a, sourceStepId: _s, lane: _l, ...params } = configNode.config;
      const allowedTriggers: Array<"stage" | "incall" | "postcall"> =
        stepAllowedTriggers[configNode.stepKey] ?? ["stage", "incall", "postcall"];
      const triggerLabels: Record<string, string> = { stage: "Stage", incall: "In-Call", postcall: "Post-Call" };

      return (
        <div className="space-y-4">
          {/* Trigger */}
          <div>
            <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Trigger</label>
            <div className="flex gap-1 flex-wrap">
              {allowedTriggers.map(t => (
                <button
                  key={t}
                  onClick={() => setDrawerTrigger(t)}
          ...
          {/* Execution Type (hidden for incall) */}
          {drawerTrigger !== "incall" && (
            <div>
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Execution</label>
              <div className="flex gap-1">
                {(["wait", "parallel"] as const).map(et => (
                  <button
                    key={et}
                    onClick={() => setDrawerExecType(et)}
                    className={`px-3 py-1 text-xs rounded-lg border font-medium transition-colors capitalize ${
                      drawerExecType === et
                        ? "bg-primary text-primary-foreground border-primary"
                        : "border-border text-muted-foreground hover:bg-muted"
                    }`}
                  >
                    {et === "wait" ? "Sequential" : "Parallel"}
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* Delay (shown only for sequential / non-incall) */}
          {drawerTrigger !== "incall" && drawerExecType === "wait" && (
            <div>
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Delay Before Step</label>
              <div className="flex gap-2">
                <input
                  type="number"
                  min={0}
                  value={drawerDelayValue}
                  onChange={e => setDrawerDelayValue(Math.max(0, parseInt(e.target.value) || 0))}
                  className="w-20 px-3 py-2 bg-input-background border border-input rounded-xl text-sm"
                />
                <select
                  value={drawerDelayUnit}
                  onChange={e => setDrawerDelayUnit(e.target.value)}
                  className="flex-1 px-3 py-2 bg-input-background border border-input rounded-xl text-sm"
                >
                  {["Second", "Minute", "Hour", "Day"].map(u => <option key={u} value={u}>{u}s</option>)}
                </select>
              </div>
              <p className="text-xs text-muted-foreground mt-1">Set to 0 for no delay.</p>
            </div>
          )}
    ...
    // Non-step nodes (start, end, manual canvas nodes)

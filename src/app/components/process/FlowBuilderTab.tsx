  ...
  useEffect(() => {
      ...
      const manualNodes = prevNodes.filter(
        n => n.id === "start" ||
             (!n.config?.autoGenerated && !n.config?.syntheticFor &&
              !n.id.startsWith("cond-") && !n.id.startsWith("parallel-"))
      );
    ...
    setConnections(prevConnections => {
      const manualConnections = prevConnections.filter(
        c => !c.id.startsWith("auto-") && !c.id.startsWith("cond-") && !c.id.startsWith("parallel-")
      );
          ...
          if (group) {
            ...
            group.steps.forEach((member, memberIdx) => {
              ...
              if (member.params?.conditionsEnabled) {
                ...
                autoConnections.push({
                  id: `cond-${condNodeId}`,
                  fromId: parallelNodeId,
                  fromPort: branchPort,
                  toId: condNodeId,
                });
                autoConnections.push({
                  id: `auto-${laneKey}-${member.id}`,
                  fromId: condNodeId,
                  fromPort: "true",
                  toId: member.id,
                });

...
export default function ManageTeamMember() {
  ...
  // Tab state
  const [activeTab, setActiveTab] = useState<"calendar" | "availability" | "days-off" | "services" | "permissions">("calendar");
            ...
            <div className="bg-white rounded-2xl border border-border shadow-sm p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Manage access levels across the platform</p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between pb-3 border-b-2 border-border">
                    <div>
                      <h4 className="text-sm font-bold" style={{ color: "#020817", fontFamily: "DM Sans, sans-serif" }}>
                        Core
                      </h4>
                      <p className="text-xs text-muted-foreground mt-0.5">Dashboard, Clients, Calls</p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => setSectionPermission("core", "view")}
                        className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                          getCurrentSectionPermission("core") === "view"
                      ...
                      <button
                        onClick={() => setSectionPermission("core", "write")}
                        className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                          getCurrentSectionPermission("core") === "write"
                            ? "bg-primary text-white shadow-md"
                            : getCurrentSectionPermission("core") === "mixed"
                            ? "bg-muted text-primary border-2 border-primary"
                            : "bg-muted text-muted-foreground hover:bg-muted/80"
                        }`}
                      >
                        Write {getCurrentSectionPermission("core") === "mixed" && "•"}
                      </button>
                    </div>
                  </div>

                  <div className="space-y-2 pl-4">
                    {["dashboard", "clients", "calls"].map((item) => (
                      <div key={item} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-muted/30">
                        <span className="text-sm font-medium capitalize" style={{ color: "#020817" }}>
                          {item}
                        </span>
                        <div className="flex gap-2">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              checked={userFormData.permissions[item as keyof ItemPermissions] === "view"}
                              onChange={() =>
                                setUserFormData({
                                  ...userFormData,
                                  permissions: { ...userFormData.permissions, [item]: "view" },
                                })
                              }
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              type="radio"
                              name={`${item}-permission`}
                              checked={userFormData.permissions[item as keyof ItemPermissions] === "write"}
                              onChange={() =>
                                setUserFormData({
                                  ...userFormData,
                                  permissions: { ...userFormData.permissions, [item]: "write" },
                                })
                              }
                              className="w-4 h-4 text-primary cursor-pointer"
                            />
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between pb-3 border-b-2 border-border">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Processes, Numbers</p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => setSectionPermission("operations", "view")}
                        className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                          getCurrentSectionPermission("operations") === "view"
                            ? "bg-primary text-white shadow-md"
                            : "bg-muted text-muted-foreground hover:bg-muted/80"
                        }`}
                      >
                      ...
                      <button
                        onClick={() => setSectionPermission("operations", "write")}
                        className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                          getCurrentSectionPermission("operations") === "write"
                            ? "bg-primary text-white shadow-md"
                            : getCurrentSectionPermission("operations") === "mixed"
                            ? "bg-muted text-primary border-2 border-primary"
                            : "bg-muted text-muted-foreground hover:bg-muted/80"
                        }`}
                      >
                        Write {getCurrentSectionPermission("operations") === "mixed" && "•"}
                      </button>
                    </div>
                  </div>

                  <div className="space-y-2 pl-4">
                    {["processes", "numbers"].map((item) => (
                      <div key={item} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-muted/30">
                        <span className="text-sm font-medium capitalize" style={{ color: "#020817" }}>
                          {item}
                        </span>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between pb-3 border-b-2 border-border">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Billing, Webhooks, Settings</p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => setSectionPermission("system", "view")}
                        className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                          getCurrentSectionPermission("system") === "view"
                            ? "bg-primary text-white shadow-md"
                            : "bg-muted text-muted-foreground hover:bg-muted/80"
                        }`}
                      >
                      ...
                      <button
                        onClick={() => setSectionPermission("system", "write")}
                        className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                          getCurrentSectionPermission("system") === "write"
                            ? "bg-primary text-white shadow-md"
                            : getCurrentSectionPermission("system") === "mixed"
                            ? "bg-muted text-primary border-2 border-primary"
                            : "bg-muted text-muted-foreground hover:bg-muted/80"
                        }`}
                      >
                        Write {getCurrentSectionPermission("system") === "mixed" && "•"}
                      </button>
                    </div>
                  </div>

                  <div className="space-y-2 pl-4">
                    {["billing", "webhooks", "settings"].map((item) => (
                      <div key={item} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-muted/30">
                        <span className="text-sm font-medium capitalize" style={{ color: "#020817" }}>
                          {item}
                        </span>

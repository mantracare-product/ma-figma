...
export function MediaFormatDropdown({
  ...
  const handleAddNewExtension = () => {
    ...
    if (result.success) {
      ...
      toast.success(`Extension "${result.formatted}" added!`);
      setSearchQuery("");
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <InfoTooltip
            text="Select allowed file extensions or type any custom extension (e.g. HEIC, SVG, DWG)."
            size="sm"
          />
        </div>
            ...
            <div className="p-2.5 bg-slate-50/90 border-b border-slate-200/80 space-y-2">
              ...
              {!exactMatchExists && cleanedQuery && (
                <button
                  ...
                  className="w-full py-1.5 px-2.5 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg text-xs font-semibold text-blue-700 flex items-center gap-1.5 transition-colors cursor-pointer group shadow-2xs text-left"
                >
              ...
              {filteredFormats.length === 0 ? (
                <div className="py-6 px-3 text-center space-y-1">
                  ...
                  <p className="text-[11px] text-slate-400">
                    Click &ldquo;Add .{cleanedQuery.toLowerCase()}&rdquo; above to add this format.
                  </p>
                </div>
              ) : (
